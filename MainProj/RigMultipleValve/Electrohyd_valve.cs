...
namespace MainProj.Local
{
    ...
    /// 电液换向阀
    /// </summary>
    public class electrohyd_valve:direction_valve
    {
        ...
        log4net.ILog logoutput = log4net.LogManager.GetLogger(typeof(electrohyd_valve));   
        protected override string 模板路径
        {
            get
            {
                return this.路径 + "\\template\\电液换向阀模板.docx";
            }
        }
        protected override string 保存路径
        {
            get
            {
                return this.路径 + "\\report\\电液换向阀\\电液换向阀试验报告.doc";
            }
        }
        ...
        private void SteadystatePreeesureSingleSide(TestValveState state)
        {
            ...
            //SetTestValveState_Pilot(TestValveState.右位);
        ...
        {
            //this.logoutput.Debug("请将球阀设置为PABT出口状态");
            //this.SetCircuitState(CircuitState.PABTOut);
            //FormWorkScope workscope = new FormWorkScope();
            //workscope.ShowDialog();
            //MessageBox.Show("如果试验对象是电液换向阀，则电磁铁试验电压设置成比额定电压低10%");
            //Group["阀4压力值缓存"] = 0f;
